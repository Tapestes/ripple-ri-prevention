...
- (void) setupCallbacks {
    ...
    [emptyView addGestureRecognizer:longpressGesture];
}

- (void)longPressHandler:(UILongPressGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        NSLog(@"TaxiDisplay Long press Began");
        if(largeView.hidden) {
            [largeView showDisplay];
        } else {
            largeView.hidden = YES;
        }
    }    
}
...
- (void) taxiPathChanged:(NSString *)text {
    textArea.text = text;
    [largeView taxiPathChanged:text];
}
